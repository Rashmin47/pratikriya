...
export default async function RoadmapPage() {
  ...
  const votesByPostId = new Map<number, number>();
  ...
  try {
    ...
    voteCounts.forEach(({ postId, _count }) => {
      votesByPostId.set(postId, _count.postId);
    });
  } catch (error) {
    console.warn("Unable to load vote counts for roadmap:", error);
  }

  const postsWithVotes = posts
    .map((post) => ({
      ...post,
      votes: Array.from({ length: votesByPostId.get(post.id) ?? 0 }),
    }))
    .sort((a, b) => {
  ...
  const groupedPosts = {
    under_review: postsWithVotes.filter((p) => p.status === "under_review"),
    planned: postsWithVotes.filter((p) => p.status === "planned"),
    in_progress: postsWithVotes.filter((p) => p.status === "in_progress"),
    completed: postsWithVotes.filter((p) => p.status === "completed"),
  };

  const totalVotes = postsWithVotes.reduce(
    (acc, post) => acc + post.votes.length,
    0,
  );
  const averageVotes =
    postsWithVotes.length > 0
      ? Math.round(totalVotes / postsWithVotes.length)
      : 0;

  // Calculate progress for the overall roadmap
  const completedPercentage = getStatusPercentage(postsWithVotes, "completed");
